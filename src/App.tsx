...
interface Penelitian {
    [index :string]: Mahasiswa
}
interface Output {
    [index :string]: Penelitian
}
...
function App() {
    ...
    const [result, setResult] = useState(
        penelitian.reduce(
            (research, { id }) => {
                const r = research;
                const a = (
                    ((r as Output)[id] = mahasiswa.reduce(
                        (acc, { id: mIdx }:{id: string}) => { const b = acc; (b as Mahasiswa)[mIdx] = 0; return b; },
                        {}
                    )),
                    r
                );
    ...
    const handleChangeValue = (e:HTMLSelectElement, pId:string, mId:string) => {
        setResult({
            ...result,
            [pId]: {
                ...(result as Output)[pId],
                [mId]: parseInt(e?.value, 10)
            }
        });
    };
    ...
    return (
        <div className="w-full container mx-auto flex flex-col gap-y-3 my-4">
            <table className="table-auto w-full text-sm text-center text-gray-500">
                ...
                <tbody>
                    {mahasiswa.map(({ id: mId, name: mahas }: {id: string, name: string}) => (
                            ...
                            {penelitian.map(({ id: pId }) => (
                                <td key={pId} className="px-6 py-4">
                                    <select
                                        ...
                                        data-value={(result as Output)[pId][mId]}
                                        onChange={(e) => handleChangeValue(e.target as HTMLSelectElement, pId, mId)}
                                    >
